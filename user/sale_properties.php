...
<head>
    ...
    <title>Properties for Sale - Latuae Land</title>
...
<body class="font-lato bg-gray-100 text-gray-800">
    <div class="container mx-auto px-4 py-6">
        <h1 class="text-xl md:text-2xl font-bold text-gray-800 text-center mb-3">Properties for Sale</h1>

        <!-- Property Grid -->
        <div class="flex justify-center">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 my-2 w-full">
                <?php if (empty($properties)): ?>
                    <p class="col-span-full text-center text-gray-600 py-5 text-sm md:text-base">Tidak ada properti untuk dijual saat ini.</p>
                <?php else: ?>
                    <?php foreach ($properties as $property): ?>
                        <a href="detail_property.php?id=<?= htmlspecialchars($property['id']) ?>" class="block h-full">
                            <div class="bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden hover:-translate-y-1 hover:shadow-lg transition h-full flex flex-col max-w-full sm:max-w-[360px] mx-auto">
